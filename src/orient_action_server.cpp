...
class OrientActionServer : public rclcpp::Node
{
public:
    using GoalHandleMoveItPose = rclcpp_action::ServerGoalHandle<MoveItPose>;

    explicit OrientActionServer(const rclcpp::NodeOptions &options)
    : Node("orient_action_server", options),
      emergency_stop_(false),
      normal_speed_(0.3),
      emergency_speed_(0.0)
    {
        RCLCPP_INFO(this->get_logger(), "Initializing Orient action server...");
        ...
        action_server_ = rclcpp_action::create_server<MoveItPose>(
            this,
            "orient_gripper",
            std::bind(&OrientActionServer::handle_goal, this, std::placeholders::_1, std::placeholders::_2),
            std::bind(&OrientActionServer::handle_cancel, this, std::placeholders::_1),
            std::bind(&OrientActionServer::handle_accepted, this, std::placeholders::_1)
        );
...
private:
    ...
    {
        RCLCPP_INFO(this->get_logger(), "Executing Orientation...");
        auto goal = goal_handle->get_goal();
        auto pose = goal->pose;
        auto result = std::make_shared<MoveItPose::Result>();
        RCLCPP_INFO(this->get_logger(), "Received Orientation: (%.3f, %.3f, %.3f, %.3f)",
